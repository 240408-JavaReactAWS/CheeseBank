...
public class TransactionController {
    ...
    public ResponseEntity<Transaction> withdrawal(@RequestBody Transaction transaction, HttpSession session) throws UserNotFoundException, InsufficientBalanceException, AccountFrozenException, InvalidTransferRecipientException {
    ...
    // Deposit
    @PostMapping("/deposit")
    public ResponseEntity<Transaction> deposit(@RequestBody Transaction transaction, HttpSession session) throws UserNotFoundException, AccountFrozenException, InsufficientBalanceException, InvalidTransferRecipientException {
        User sessionUser = (User) session.getAttribute("user");
        if (sessionUser == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
    ...
    public ResponseEntity<Transaction> transfer(@RequestBody Transaction transaction, HttpSession session) throws UserNotFoundException, InsufficientBalanceException, AccountFrozenException, InvalidTransferRecipientException {
        User sessionUser = (User) session.getAttribute("user");
        if (sessionUser == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
