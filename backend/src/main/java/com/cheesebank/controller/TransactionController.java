...
@CrossOrigin(origins = "${frontend.url}", methods = { RequestMethod.GET, RequestMethod.POST, RequestMethod.PUT,
        RequestMethod.PATCH, RequestMethod.DELETE }, allowCredentials = "true")
public class TransactionController {
    ...
    public ResponseEntity<Transaction> withdrawal(@RequestBody Transaction transaction, HttpSession session)
            throws UserNotFoundException, InsufficientBalanceException, AccountFrozenException {
    ...
    // Deposit
    @PostMapping("/deposit")
    public ResponseEntity<Transaction> deposit(@RequestBody Transaction transaction, HttpSession session)
            throws UserNotFoundException, AccountFrozenException, InsufficientBalanceException {
        User sessionUser = (User) session.getAttribute("user");
        if (sessionUser == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
    ...
    // public ResponseEntity<Transaction> transfer(@RequestBody Transaction
    // transaction, HttpSession session) throws UserNotFoundException,
    ...
    // to " + targetUser.getFirstName() + " " + targetUser.getLastName() + "'s
    // account");
    // return ResponseEntity.status(HttpStatus.CREATED).body(transaction);
    // }

    @PostMapping("/transfer/{emailTo}")
    public ResponseEntity<Transaction> transfer(@PathVariable String emailTo, @RequestBody Transaction transaction,
            HttpSession session) throws UserNotFoundException, InsufficientBalanceException, AccountFrozenException {
    ...
    // viewTransactionHistoryByUsername(@PathVariable String username, HttpSession
    // session, Pageable pageable) throws UserNotFoundException {
    // User sessionUser = (User) session.getAttribute("user");
    // if (sessionUser == null) {
    // System.out.println("Unauthorized access");
    // return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
    ...
    // System.out.println("Successfully retrieved " + searchedUser.getFirstName() +
    // " " + searchedUser.getLastName() + "'s past transactions");
    // return ResponseEntity.ok(transactionService.getAllTransactions(searchedUser,
    // pageable));
    // }

    @GetMapping("/history/{username}")
    public ResponseEntity<List<Transaction>> viewTransactionHistoryByUsername(@PathVariable String username,
            HttpSession session, Pageable pageable) throws UserNotFoundException {
        ...
        User searchedUser = user.get();
        System.out.println("Successfully retrieved " + searchedUser.getFirstName() + " " + searchedUser.getLastName()
                + "'s past transactions");
    ...
    public ResponseEntity<Page<Transaction>> filterByTimeRange(@PathVariable LocalDateTime startDate,
            @PathVariable LocalDateTime endDate, HttpSession session, Pageable pageable) {
        User sessionUser = (User) session.getAttribute("user");
        if (sessionUser == null) {
            System.out.println("Unauthorized access");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        ...
        return ResponseEntity
                .ok(transactionService.getTransactionsByTimeRange(sessionUser, startDate, endDate, pageable));
    }

    // View all transactions of a type
    @GetMapping("/history/type/{type}")
    public ResponseEntity<Page<Transaction>> filterByType(@PathVariable String type, HttpSession session,
            Pageable pageable) {
        User sessionUser = (User) session.getAttribute("user");
        if (sessionUser == null) {
            System.out.println("Unauthorized access");
